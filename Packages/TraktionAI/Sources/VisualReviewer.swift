...
/// adapters only; no provider SDK, no pixel authority. Decisions mirror
/// `config/visual-review-decision.schema.json` and are recommendations that a
/// deterministic validator must accept or reject before they change output
/// behavior (docs/ARCHITECTURE.md architectural rule).
public struct VisualReviewRequest: Codable, Equatable, Sendable {
    /// The joint or region the reviewer is being asked about.
    public let targetID: String
    /// Deterministic evidence summary prepared by the engine. Never full
    /// output pixels; cropped diagnostic regions only (AGENTS.md privacy).
    public let evidenceSummary: String

    public init(targetID: String, evidenceSummary: String) {
...
/// Default reviewer: always abstains. Core reconstruction must work with this
/// implementation alone (offline invariant).
public struct DisabledVisualReviewer: VisualReviewer {
    ...
    public func review(_ request: VisualReviewRequest) async throws -> VisualReviewDecision {
        VisualReviewDecision(
            ...
            confidence: 0,
            abstain: true,
            reasonCode: "reviewer_disabled"
        )
    }
}

/// Test reviewer returning scripted decisions keyed by target ID.
public struct MockVisualReviewer: VisualReviewer {
    ...
    public func review(_ request: VisualReviewRequest) async throws -> VisualReviewDecision {
        decisions[request.targetID] ?? VisualReviewDecision(
            action: .noChange,
            confidence: 0,
            abstain: true,
            reasonCode: "mock_unscripted_target"
        )
    }
}
