        ...
        ScrollView {
          VStack(alignment: .leading, spacing: 16) {
            if let result = model.result {
              ...
              if model.joint != nil { sourcePicker }
              ...
              zoomControls
              panControls
              Text("1:1 shows one source pixel per display pixel. Drag the image to pan, or use the direction buttons. Inspection is read-only.")
                .font(.caption)
